...
def FISTA(
    ...
    if v_init is not None:
        ...
        v = torch.full_like(y_target, y_target.max().item() / 2.0, requires_grad=True)

    x_prev = v.clone()

    loss_hist = []
    for epoch in tqdm(range(iters)):
        y_hat = forward_model(v)
        if y_hat.shape != y_target.shape:
            y_hat = center_crop(y_hat, y_target.shape[-2:])
        loss = torch.linalg.norm(y_target - y_hat)
        if reg_fn is not None:
            loss = loss + reg_fn(v)

        g = torch.autograd.grad(outputs=loss, inputs=v)[0]
        # Gradient update step
        v = v - lrate * g
        # Projection step (non-negativity constraint)
        x = v.clamp(0, float('inf'))
        # Ts update
        t[-1] = (1 + sqrt(1 + 4 * (t[0]**2)))/2.0
        # Momentum update
        v = x + ((t[0]-1)/t[1])*(x - x_prev)
        ...
        if save_path is not None and (epoch + 1) % (iters//20) == 0:
            to_pil_image((v / v.max()).detach().squeeze().cpu()).save(save_path + f'/recon_{epoch}.png')
            np.save(save_path + f'/recon_{epoch}.npy', x.detach().squeeze().cpu().numpy())
